...
//
// About disabling @typescript-eslint/ban-types:
// Using `object` is actually correct here since that’s exactly what `new Proxy()` takes.
const proxyFileIIFE = (scope: Record<string, unknown>): void => {
  // This is a stub file generated by `elm-watch`.
  //
  // It generates `window.Elm` in a way that should make your app do nothing,
  // and not crash.
  //
  // It tries to connect to `elm-watch`'s WebSocket server as quickly as
  // possible and reload the page as soon as the real thing has been compiled.
  //
  // Generating these stub files until a WebSocket connects and proves that the
  // actual compiled Elm code is needed, is a big performance optimization.
  ...
  const moduleProxy = (proxied?: object): unknown =>
    new Proxy(proxied ?? { init: (_: unknown) => ({ ports: portsProxy() }) }, {
      get: (target, property, receiver) =>
        Reflect.has(target, property) ||
        typeof property === "symbol" ||
